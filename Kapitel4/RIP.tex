...
Sei schlieﬂlich
\[\Sigma_k^m := \menge{x \in\R^m}{\abs{\menge{i\in\haken m}{x_i\neq 0}}\le k}\]
...
\end{satz}
\begin{proof}
Seien $(E, \norm\cdot_E):= \ell_2^N$, $T:= \Sigma_k^m\cap S^{m-1}$ und
$x := \left(e_i^N\right)_{i=1}^N$.\\
...
Da $\abs{\menge{i \in \haken m}{s_i\neq 0}} \le k$, gilt ($\mu_{j,n}$ gem‰ﬂ
\ref{ktesMaximum} f¸r alle $j \in \haken n$):
\begin{align*}
...
\end{align*}
Offenbar ist $\eta \in \!\  ]0,1[$ und mit $(a), (b), (c)$ sind die Voraussetzungen von \ref{ExistenzEingeschraenkterFastIsometrien} erf¸llt,
