...
\end{satz}
\begin{proof}
Zunächst sind offenbar die Voraussetzungen von
\ref{GordonschePaareUndSubeuklidischeBildhalbnormen} erfüllt.\\
Seien
...
Seien nun $a:=\E(Z_{\gamma,x}) - \E(M_{\delta, T})$ und 
$b:=\E(Z_{\gamma,x}) + \E(M_{\delta, T})$.\\
Wegen $f \ge 0$ und $h \ge 0$ und nach obiger Rechnung gilt:
\[ 0 < a \le \E(f) \le E(h) \le b\text.\]
Mit der Integrierbarkeit von $f$ und $h$ sind somit die Voraussetzungen von
\ref{Erwartungswertlemma} für $f, h, a, b$ erfüllt und für
$Q :=\menge{\omega \in \Omega}{f(\omega)> 0 \wedge ah(\omega) \le bf(\omega)}$
...
und $S^{m-1} = \kran{\ell^2_m}01$ nach 
\ref{InjektivitätLinearerAbbildungenZwischenNormiertenRäumen} die Injektivität
von $A$.\\
Weiter ist der Raum 
...
Dann ist $A$ bijektiv, linear und stetig, sodass nach
\ref{OperatornormDerInversen} gilt:
