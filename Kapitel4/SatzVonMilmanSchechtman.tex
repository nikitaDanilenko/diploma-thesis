...
Sei $N := \floor{\frac n 2}$. Dann ist $N \le \frac n 2 < \widetilde M$ und
daher $x:=\left(\frac 1{C_{BS}}\widetilde y_i \right)_{i=1}^N$ wohldefiniert
...
\ref{ExistenzNormalNTupel}), $G_x := \bprod \cdot x_N \circ g$ und $(Z_{\gamma, x}, M_{\delta,T})$ das
...
und
\[ V:= \mengea{z \in \R^N\setminus\meng 0}
...
\frac{15}2 > \exp(1)}
\sqrt{\log\left(1 + \frac {n}m \right)} =: \alpha \text.\label{svmseq:6}
\end{align}
Seien $E_1:= \R^N$, $\norm\cdot_{E_1}:= \norm\cdot_{(m)}$,
$x_1 := (e_i^N)_{i=1}^N$ und $G_{x_1}:= \bprod \cdot {x_1}_N \circ g = g$.\\
Wegen $(a), (b), (c)$ sind die Voraussetzungen von
\ref{GordonschePaareUndNormiterierendeMengen} für $(E_1, \norm\cdot_{E_1})$,
$x_1$, $T$, $V$ und $c:=\frac 1 {\sqrt{m}}$ erfüllt und damit gilt:
\begin{align}
\E\left(\norm\cdot_{(m)} \circ \gamma \right) - \frac {a_m}{\sqrt m}
...
Sei $A:= G_x(\omega_0)$. Dann ist $A \in L(\ell_2^m, E)$ und es gilt:
\[\inf_{t \in S^{m-1}}\norm{At}_E =
...
Sei $F:= \Bild A$. Da $A$ injektiv ist, ist $\dim F = m$ und da jeder
...
Sei dann zusätzlich $\eps := \frac 1 2$.\\
