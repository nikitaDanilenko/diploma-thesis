...
\end{satz}
\begin{proof}
Seien $(E, \norm\cdot_E):= \ell_2^N$, $x := (e_i^N)_{i=1}^N$ und
\[T:= \mengea{\frac{v(i)-v(j)}{\norm{v(i)-v(j)}_{2,m}}}{i, j \in \haken n,
\, i \neq j}\text{.}\]
...
$\sigma_i:=1$ f¸r alle $i \in \haken k$) gilt daher
...
\end{align}
Sei nun
$\mca := \menge{\frac{1-\eps}{\inf\menge{\norm{Bt}_{2,N}}{t \in T}}B}
{B \in \mcb}$.\\
...
\end{align*}
Auﬂerdem gilt vollkommen analog:
\begin{align*}
&\,\frac{\norm{A(v(i)) - A(v(j))}_{2,N}}{\norm{v(i)-v(j)}_{2,m}}=
\frac {1-\eps}\alpha \norm{B(t_{ij})}_{2,N}
