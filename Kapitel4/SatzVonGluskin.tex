...
Seien $m \in \N$, $\eps \in\!\ ]0, 1[$, $N:= \ceil{(1-\eps)m}$ und
\[c := 2\sqrt \pi (\sqrt 2 +1) \sqrt{2 + \frac 4{\log(4)}}
...
\end{satz}
\begin{proof}
Seien $\rho := (\sqrt 2 +1)^2\left(2+\frac 4{\log(4)} \right)$,
$\eta := \frac 2 \pi \frac 1 \rho (1-\eps)$ und
$t_\eta := \frac 4 \eta \log\left(\frac 2 \eta \right)$.\\
...
\end{align}
Falls $N=m$ gilt, ist die Aussage trivial, sodass wir $N<m$ annehmen kˆnnen.\\
\underline{1. Fall:} $m \le 2 t_\eta$.\\
Sei $F := \left\langle \menge{e_i^m}{i \in \haken {m-N}}\right\rangle$.\\
Offenbar ist dann $(F,\norm\cdot_1\vert_{F})$ isometrisch isomorph zu
$\ell_1^{m-N}$ und damit nach \refclaim{EigenschaftenDesBanachMazurAbstands}
{EigenschaftenDesBanachMazurAbstands2}:
$d_{BM}((F,\norm\cdot_1\vert_{F}), \ell_1^{m-N}) = 1$.\\
Weiter gilt nach
...
d_{BM}((F,\norm\cdot_1\vert_{F}), \ell_1^{m-N})d_{BM}
(\ell_1^{m-N}, \ell_2^{m-N})\\
...
Seien $(E,\norm\cdot_E):=\ell_2^N$, $T:=S^{m-1}\cap\kabg{\ell_1^m} 0{\sqrt k}$,
$x := (e_i)_{i = 1}^N$,
...
$t_\eta$ gem‰ﬂ \refclaim{Numeriklemma}{Numeriklemma4}:
\begin{align}
...
Wegen $k \ge \frac 1 2 \frac m {t_\eta}$ gilt schlieﬂlich
\begin{align*}
