...
\usepackage{makeidx}
\usepackage{eurosym}
%\usepackage{ulsy}  % Not available in TeX Live; not used in this document
\usepackage{byname}
\usepackage{nameref}
...
unter Zuhilfenahme des Gauß'schen Min--Max--Satzes zu beweisen. Um dieses zu
bewerkstelligen, wird zunächst der besagte Satz auf eine bestimmte Klasse
...
Satz über die "`Restricted Isometry Property"'.
...
Quellen. Ebenso gehen die Folgerungen aus dem ursprünglichen Gauß'schen
...
linearen Abbildungen und insbesondere Funktionalen beweisen. Die meisten dieser
Aussagen sind elementarer Natur, einige allerdings benutzen bekannte Sätze der
...
\input{Kapitel1/DefinitionLinearkombinator}
\parag
Auch hier ergeben sich einige einfache Eigenschaften.

\input{Kapitel1/EigenschaftenDesLinearkombinators1}

\input{Kapitel1/EigenschaftenDesLinearkombinators2}
\parag
...
\input{Kapitel1/HinreichendeBedingungFuerDieInjektivitaetLinearerAbbildungen}
...
\input{Kapitel1/NotwendigeBedingungFuerDieInjektivitaetLinearerAbbildungen}
\parag
...
\input{Kapitel1/OperatornormDerInversen}
\parag
...
\input{Kapitel1/DefinitionBildhalbnorm}
...
\input{Kapitel1/EigenschaftenDerBildhalbnorm}
\parag
...
\input{Kapitel1/InvertierbarkeitDesGrenzwertsInvertierbarerOperatoren}
\parag
...
gelten möge, um keine weitere Forderung stellen zu müssen. In sämtlichen späteren Anwendungen (vor allem im endlichdimensionalen Fall) wird die erwähnte
Menge stets nichtleer sein.

\input{Kapitel1/DefinitionBanachMazurAbstand}
\parag
Die Normiertheitsbedingung einer Banach--Mazur--Abbildung dient lediglich der
Bequemlichkeit. Als nächstes notieren wir einige Eigenschaften des
Banach--Mazur--Abstandes.

\input{Kapitel1/EigenschaftenDesBanachMazurAbstands}

Wir stellen uns nun die Frage, ob es stets eine Banach--Mazur--Abbildung
zwischen zwei isomorphen Räumen gibt. Im Unendlichdimensionalen braucht es
diese Abbildungen nicht zu geben, im Endlichdimensionalen hingegen existieren
stets Banach--Mazur--Abbildungen.

\input{Kapitel1/ExistenzEinerBanachMazurAbbildung1}
\parag
Im Beweis ist eingegangen, dass $(E, \norm\cdot_E)$
ein Banachraum ist, $\Inv(E, F)\neq \emptyset$ gilt und $\kran{L(E,F)}01$
kompakt ist. Die beiden letztgenannten Voraussetzungen implizieren aber die
...
\input{Kapitel1/KompaktheitDerSphaere}
\parag
...
\input{Kapitel1/Normaequivalenzbasis}
\parag
...
\input{Kapitel1/DvoretzkyRogersLemma}
\parag
...
\input{Kapitel1/AequivalenzkonstantenFuerPNormen}
\parag
...
\input{Kapitel1/StetigeFunktionenUndDichteTeilmengen}

\input{Kapitel1/DichtheitUndHaeufungspunkte}
...
\input{Kapitel1/GleichmaessigeLipschitzstetigkeit}

\input{Kapitel1/GeradeUngeradeFunktionen}
\parag
...
\input{Kapitel1/Sortierungsmengen}
\parag
...
\[\mcm_n(\Omega, \mfa, \R^n) 
 :=\menge{f\in\mcf(\Omega,\R^n)}{f\quad \mfa - \mfb_n - \text{messbar}}\text.\]
Schließlich sei für alle $n \in \N$:
\[\mct_n(\Omega, \mfa, \R^n)
:= \menge{f \in \mcm_n(\Omega, \mfa, \R^n)}{\Bild f \text{ endlich}}\text.\]
Auch hier treffen wir die Vereinbarung, dass für $n = 1$ der Index weggelassen
werden möge.

\input{Kapitel2/DefinitionUnabhaengig}
...
\input{Kapitel2/ElementareEigenschaftenQuadratintegrierbarerAbbildungen}
\parag
...
\input{Kapitel2/UnkorreliertheitImLinearkombinatorZweifach}
\parag
...
\input{Kapitel2/UnkorreliertheitImLinearkombinatorEinfach}
\parag
...
\input{Kapitel2/Integrierbarkeitskriterien}
\parag
...
\input{Kapitel2/MonotoneKonvergenzImIntegrierbarenFall}
\parag
...
\input{Kapitel2/StrengeMonotonieDesErwartungswerts}
\parag
...
\input{Kapitel2/Erwartungswertlemma}
\parag
...
\input{Kapitel2/ExistenzFVerteilterZufallsgroessen}

\input{Kapitel2/NormalNTupel}

\input{Kapitel2/NormalverteilteTupel}
\parag
...
\input{Kapitel2/ErwartungswertDer2NormEinesStandardnormalverteiltenProzesses}
\parag
...
\input{Kapitel2/AbschaetzungEinesGammafunktionsquotienten}
\parag
...
verallgemeinern.
\input{Kapitel2/EndlicheDarstellungDesSupremumsIntegrierbarerFunktionen}
\parag
...
\input{Kapitel2/EndlicheDarstellungDesInfimumsIntegrierbarerFunktionen}

\input{Kapitel2/AbzaehlbareDarstellungUndInfimumVonSuprema}

\input{Kapitel2/AbzaehlbareDarstellungUndSupremumVonSuprema}
\parag
...
\input{Kapitel2/Ordnungsstatistik}
\parag
...
\input{Kapitel2/EigenschaftenVonOrdnungsstatistiken}

Um konkrete Abschätzungen der Erwartungswerte gewisser absoluter
Ordnungsstatistiken anzugeben kann man deren Verteilung bzw. Dichte
benutzen. Es wird sich als nützlich herausstellen, die Abbildung
$\Prim(\abs{\gamma} > \cdot)$ statt der Verteilung zu betrachten. Im Falle
einer $N(0, 1)$--verteilten Zufallsgröße $\gamma$ hat diese Abbildung
besonders gute Eigenschaften, die wir als nächstes notieren. Die erste
Ungleichung der vierten Aussage beruht dabei maßgeblich auf \cite{RW}, die
Anwendung auf die komplementäre Verteilungsfunktion ist elementar.

\input{Kapitel2/EigenschaftenDerKomplementaerenVerteilungsfunktionVonN01}
\parag
...
\input{Kapitel1/AbschaetzungDerUnvollstaendigenBetafunktion}
\parag
Wir sind nun imstande, Schranken für den Erwartungswert der $k$--ten absoluten
Ordnungsstatistik eines standardnormalverteilten $\gamma$ anzugeben. Es stellt
sich heraus, dass die obere und untere Schranke sich im Wesentlichen nur um
...
\input{Kapitel2/UntereAbschaetzungDesErwartungswertsDerKtenAbsolutenN01Ordnungsstatistik}
\parag
...
\input{Kapitel2/ObereAbschaetzungDesErwartungswertsDerKtenAbsolutenN01Ordnungsstatistik}
...
\newpage
\input{Kapitel2/GaussscherMinMaxSatz}
...
\input{Kapitel2/GaussscherMinMaxSatzGleichheitInDerErstenBedingung}

\input{Kapitel2/SatzVonFernique}
...
optimal ist. In dem folgenden Korollar kommt noch hinzu, dass die
Beweisstrategie vermuten lässt, man würde grob schätzen, während die erhaltene
Abschätzung allerdings tatsächlich optimal ist. Das folgende Korollar ist eine
Modifikation einer in \cite{Gor2} vorgenommenen Nebenrechnung.

\input{Kapitel2/MaximumNormalverteilterZufallsgroessen}
...
Min--Max--Satzes zulassen. Die Namensgebung geht auf Yehoram Gordon zurück, der
...
unabhängig. Sei weiter $g:= \Mat_{m,N} \circ \widetilde g$.\\
...
\input{Kapitel3/IntegrierbarkeitUndVerteilungVonLinearkombinationen}
...
\input{Kapitel3/GaussscherOperator}
...
sind. Damit ist die folgende Setzung wohldefiniert.

\input{Kapitel3/GordonschesPaar}
\parag
Als nächstes notieren einige elementare Eigenschaften Gordon'scher Paare.

\input{Kapitel3/EigenschaftenGordonscherPaare}
\parag
Für die Prüfung der Voraussetzungen im Gauß'schen Min--Max--Satz sind die
$L_2$--Abstände innerhalb Gordon'scher Paare von besonderem Interesse. Mit dem
eben bewiesenen Lemma können diese einfach bestimmt werden. Diese Abstände wurden bereits von Gordon in \cite{Gor}, \cite{Gor2} in modifizierter Weise
bestimmt.
\newpage
\input{Kapitel3/AbstaendeInGordonschenPaaren}
\parag
Da das "`Min--Max"' aus dem Gauß'schen Min--Max--Satz für Gordon'sche Paare
sinnvollerweise zu einem "`Inf--Sup"' umgewandelt werden sollte, brauchen wir
die Beschränktheit gewisser Mengen, welche wir in dem nachfolgenden Lemma
notieren.

\input{Kapitel3/BeschraaenktheitGordonscherPaare}
\parag
...
Mengen verhalten. Die Antwort ist nicht sehr überraschend -- die betrachteten
Infima bzw. Suprema stimmen darauf mit denen der ganzen Menge überein.

\input{Kapitel3/GordonschePaareAufDichtenTeilmengen}
\parag
...
Infima und Suprema zunächst als abzählbare Extrema darzustellen. Daher können
wir nun einfach die Messbarkeit dieser Extrema im Falle beschränkter
$T,V$ einsehen.

\input{Kapitel3/MessbarkeitVonExtremaInGordonschenPaaren}
\parag
...
\input{Kapitel3/IntegrierbarkeitVonExtremaInGordonschenPaaren}
\parag
...
\input{Kapitel3/GordonschePaareUndDerGaussscheMinMaxSatzEndlich}
\parag
...
der letzte Satz nicht direkt anwendbar ist. Ist allerdings $0$ auch ein
Häufungspunkt von $V$, so kann man zu einer dichten Teilmenge übergehen, die
$0$ nicht enthält. Auf die Voraussetzung an die $0$ als Häufungspunkt kann
nicht verzichtet werden, weil z. B. für $V := \meng 0$, $T:=\meng{e_1^m}$ und
$c := 1$ im die anderen Voraussetzungen offensichtlich gegegeben sind, aber
...
\input{Kapitel3/GordonschePaareUndDerGaussscheMinMaxSatz}
\parag
Die Idee, den Gauß'schen Min--Max--Satz auf Gordon'sche Paare anzuwenden
stammte von Gordon. Tatsächlich sind die Gordon'schen Paare genau so
...
(und wurde in \cite{Gor2} verwendet). In der vorliegenden Form wird es in
dieser Arbeit vorrangig für den Beweis des Satzes von Milman und Schechtman
verwendet. Dort werden wir die Normdarstellung durch ein Supremum auf die
spezielle Norm aus \ref{Sortierungsnorm} anwenden.

\input{Kapitel3/GordonschePaareUndNormiterierendeMengen}
Das nächste Korollar ist in der vorgestellten Form eine Nebenrechnung aus
\cite{Gor2} (verkürzt auch in \cite{Gor} verwendet).

\input{Kapitel3/GordonschePaareUndSubeuklidischeBildhalbnormen}
...
\input{Kapitel4/Auswertungspaar}
...
Abbildungen zuzusichern. Die Menge $T$ wird die Eigenschaften dieser
Abbildungen regulieren. So wird zum Beispiel $T = S^{m-1}$ die Abbildungen
injektiv machen.

Wir sind nun in der Lage, einen wichtigen Satz der Banachraumtheorie zu
beweisen. Dieser sichert die Existenz gewisser linearer Abbildungen zu,
welche sich besonders gut dazu eignen, den Banach--Mazur--Abstand zweier
Räume sehr einfach abzuschätzen.

\input{Kapitel4/ExistenzEingeschraenkterFastIsometrien}
\parag
...
$T := S^{m-1}$ setzen. Es stellt sich allerdings heraus, dass die
Voraussetzungen an das $x$ und das Auswertungspaar mit Hilfe des
Banach--Mazur--Abstandes implizit ausgedrückt werden können. Die dadurch
entstehende Bedingung ist einerseits handlicher, weil es eine einzige
Ungleichung ist, andererseits weniger einfach, weil in der Anwendung der
Banach--Mazur--Abstand bestimmt werden müsste. Wir fassen dies in
dem folgenden Korollar zusammen.

\input{Kapitel4/ExistenzFastIsometrischerAbbildungen}
...
\input{Kapitel4/UnterraeumeVonlp}
\parag
...
als (in gewisser Hinsicht) Verschärfung dieser Vergleiche kennen gelernt.
Der auf dem Dvoretzky--Rogers--Lemma beruhende Satz von Dvoretzky ermöglicht es nun, unter einer sehr ähnlichen Bedingung wie in \refclaim{AbstaendeBezueglichDerPnorm}{AbstaendeBezueglichDerPnorm2} den
...
\input{Kapitel4/SatzVonDvoretzky}
\parag
Der wesentliche Aspekt des Beweises ist die Existenz eines $x \in E^n$ mit
der verwendeten Abschätzung nach oben und unten. In der
Dvoretzky--Rogers--Variante kann diese Abschätzung nicht weiter verbessert
werden, weil die Voraussetzung an das $n$ bzw. $N$ wesentlich in die
Abschätzung eingeht. Allerdings gibt es abstraktere Verbesserungen, wie die
...
bei Dvoretzky--Rogers) ist allerdings schlecht explizit bestimmbar, weil sie
sich aus axiomatisch existenten Faktoren zusammensetzt. Nichtsdestoweniger
...
\input{Kapitel4/JohnsonLindenstraussLemma}
\parag
Wir können also eine endliche Menge von normierten Vektorendifferenzen so
abbilden, dass alle Bilder "`fast"' normiert sind. Unter etwas schärferen
Voraussetzungen können aber auch bestimmte unendliche Mengen (nicht
notwendigerweise spezieller Differenzen) ähnlich abgebildet werden.

\input{Kapitel4/RIP}
\parag
Die "`Ausdünnung"' der Vektoren in diesem Satz lässt höchstens ein Drittel der
Komponenten von $0$ verschieden. Auch wenn dies zunächst sehr schwach erscheint
...
Menge solcher Operatoren. Der Grund für diese Abschwächung ist
der, dass die "`Größe"' der Menge sich nicht ohne den Gauß'schen Operator auf
...
Gauß'schen Min--Max--Satzes überraschend einfach beweisen lässt. Dieses ist der
Satz von Gluskin, welcher besagt, dass es in $\ell_1^m$ zu $N \in \haken m$
stets $N$--codimensionale Unterräume gibt, deren Abstand zum $\ell_2^{m-N}$ von
der Größenordnung $\sqrt{\frac m N \log\left(\frac{2m}N\right)}$ ist. Eine
konkrete (wenn auch recht grobe) Abschätzung ist mit unseren Methoden ebenfalls
möglich.

\input{Kapitel4/SatzVonGluskin}

Die Konstante $c$ in dem Satz ist in etwa $48.2$. Beachtet man, dass die
Abschätzungen für die Ordnungsstatistiken eher kanonisch als filigran waren, so
ist es naheliegend anzunehmen, dass diese Konstante etwas verbessert werden
kann. Ferner ist auch die Abschätzung des $t_\eta$ aus dem Numeriklemma (vgl.
...
mit Hilfe von \ref{ExistenzEingeschraenkterFastIsometrien} bewiesenen Sätzen
...
{ExistenzEingeschraenkterFastIsometrien2}. Hierbei bestand die Aufgabe darin,
...
Zum Abschluss beweisen wir den Satz von Milman und Schechtman. Dieser stellt
eine interessante Ergänzung zum Satz von Dvoretzky dar, auf die im Anschluss
an den Beweis eingegangen wird.

Der vorgestellte Beweis beruht auf jenen von Gordon in \cite{Gor2}
und Guédon in \cite{Gued}. Die Idee, die in \ref{Sortierungsnorm} vorgestellte
...
\input{Kapitel4/SatzVonMilmanSchechtman}
...
Gluskin recht klein). Die Aufteilung des Beweises in die verschiedenen Fälle
...
noch mit Konstante $1$ erfüllt. Abstrahiert man sich von diesem wohlwollenden
...
Konstante vor dem logarithmischen Term. Zu beachten gilt aber auch, dass die
...
zum Satz von Dvoretzky dar. Um diesen Sachverhalt genau notieren zu können,
...
\end{align*}
Seien nun $c\in\!\ ]0,\nicefrac 12[$ und $n\in\N$. Dann gilt:
\begin{align*}
...
besitzt nach den Zwischenwertsatz eine Nullstelle $c_0 \in \R$ (und auch
genau eine, weil $h$ streng monoton wachsend ist). Offensichtlich ist
...
Um eine Größenordnung des $n$ zu erhalten, betrachten wir $c_1:=\frac 9{20}$.\\
Es gilt $h(c_1) = \frac{59049}{3200000}+ \frac 9 {20}- \frac 1 2 = 
- \frac{100951}{3200000}$ (also $c_1$ "`nahe bei"' $c_0$).\\
...
$c_2:= \frac{476}{1000}$ erhält man sogar $\N_{\ge 1737}$).
