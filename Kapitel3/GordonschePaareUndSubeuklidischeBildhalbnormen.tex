...
\end{korollar}
\begin{proof}
(1) Sei $V:= \menge{\pfeil \vi N (x)}{\vi \in \kabg{E'}01}$.\\
...
Sei $v \in V$. Dann gibt es ein $\vi \in \kabg{E'}01$ mit $v=\pfeil\vi N(x)$.\\
Ferner gilt:
\begin{align*}
\norm v_{2, N}^2 &= \sprod v v_N = \sprod v {\pfeil \vi N(x)}_N
\overset{\refclaim{EigenschaftenDesLinearkombinators}
{EigenschaftenDesLinearkombinators2}}=
\vi\left(\bprod v x_N \right) \le \abs{\vi\left(\bprod v x_N \right)}\\
&\le \opnorm \vi E \R\norm{\bprod v x_N}_E\overset{\vi \in \kabg{E'}01}\le
\norm{\bprod v x_N}_E \overset{\text{Vor.}}\le \norm v _{2, N}
\end{align*}
Damit ist $\norm v _{2, N} \le 1$.\\
Daher ist $c:=1$ eine obere Schranke von $\menge{\norm w _{2, N}}{w \in V}$.
\hfill $(\ast)$\\
...
Definiere $v_n := \frac 1 n \pfeil \alpha N (x)$ für alle $n \in \N$.
...
\end{align*}
Damit sind wegen $(\ast)$ die Voraussetzungen von
\ref{GordonschePaareUndNormiterierendeMengen} mit $c=1$ erfüllt und die Behauptung ist dann genau die Aussage
