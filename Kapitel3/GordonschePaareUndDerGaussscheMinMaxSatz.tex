...
Sei also
\[V' := \begin{cases}\widetilde V &:\text{ falls } 0 \notin V\\
V'' & : \text{ sonst} \end{cases}\text.\]
Dann ist insbesondere $0 \notin V'$.\\
Seien $\mct := \Pot_{ne}(T')$ und $\mcv:= \Pot_{ne}(V')$.\\
Nach
...
\end{align*}
Seien $I:=T'$, $J:=V'$, $X':= X\vert_{I\times J} = X\vert_{T' \times V'}$ und
$Y':=Y\vert_{I \times J}= Y\vert_{T' \times V'}$.\\
