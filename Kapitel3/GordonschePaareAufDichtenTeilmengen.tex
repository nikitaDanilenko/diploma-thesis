...
$\emptyset \neq V \subseteq \R^N$ beschränkt, sowie $T' \subseteq T$
...
Da $V$ beschränkt ist, gibt es $d \in \R_{>0}$ mit 
$\forall\,v \in V: \norm v_{2, N} \le d$.\\
Sei $\omega \in \Omega$.\\
(1) Die Abbildung $\sprod \cdot {\gamma(\omega)}_N$ ist lipschitzstetig und da
lipschitzstetige Bilder beschränkter Mengen beschränkt sind, ist die Menge
$\sprod \cdot {\gamma(\omega)}_N(V)$ beschränkt.\\
...
$(c \sprod \cdot {\delta(\omega)}_m)(T)$ nach oben und unten beschränkt.\\
...
\end{align*}
Weiter gilt wegen der Beschränktheit von $T, T', V, V'$:
\begin{align*}
...
\end{align*}
(3)/(4) Für jedes $w\in V$ ist die Abbildung $\sprod \cdot{g(\omega)^\top w}_m$
lipschitzstetig und linear und es gilt für alle $x \in \R^m$:
\begin{align}
...
\label{gpadteq:1}
\end{align}
Somit ist $\menge{\sprod x {g(\omega)^\top v}}{v \in V}$ beschränkt
und $\left(\sprod \cdot {g(\omega)^\top v}_m \right)_{v \in V}$ eine
Familie gleichmäßig lipschitzstetiger Abbildungen.\\
...
Da $T$ beschränkt ist, ist also auch $f(T)$ beschränkt, sodass gilt
\begin{align*}
...
= \left(\sup_{t \in T}\sup_{v \in V'}Y(t, v) \right)(\omega)
\end{align*}
Damit ist zunächst (3) gezeigt.\\
Für jedes $s \in T$ ist die Abbildung $\sprod {g(\omega)s}\cdot_N$
