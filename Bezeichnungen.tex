Seien $n,m\in \N$, $p \in [1, \infty[$, $K$ ein Körper, $\K\in\meng{\R, \mathbb C}$.\\
Wenn wir von einem normierten Vektorraum reden, setzen wir implizit voraus,
dass dieser nicht der Nullvektorraum ist.

\begin{enumerate}[(1)]
 \item \label{Bez1}
       Wir schreiben verkürzend $\haken n \coloneqq  \N_{\le n}$ und
       $\haken n _0\coloneqq \haken n \cup \meng 0$.
 \item \label{Bez2}
 ...
 \item \label{Bez13}
       ...
       $\Pot_{ne}(M)\coloneqq \menge{A\in\Pot(M)}{A\neq\emptyset,\,A\text{ endlich}}$.
 \item \label{Bez4}
 ...
 \item \label{Bez7}
       ...
       alle $i \in \haken n$: $f_i \coloneqq  \text{pr}_i \circ f$, wobei $\text{pr}_i$
 ...
 \item \label{Bez8}
       ...
       Für $A\in K^{n\times m}$ definieren wir $A_i \coloneqq  A e_i^n$ und
       $A_{i,j}\coloneqq \sprod{A_i}{e_j^m}_m$ für alle $i\in\haken n$,
 ...
 \item \label{Bez10}
       ...
       Ferner sei
       \[\Inv(X, Y) \coloneqq  \menge{T \in L(X, Y)}{T \text{ invertierbar und }
       T\inv \in L(Y, X)}\]
 \item \label{Bez11}
       Ist $X$ ein $K$--Vektorraum, so seien $X^* \coloneqq  \Hom(X,\K)$ (algebraischer
       Dualraum) und $X'\coloneqq L(X, \K)$ (stetiger Dualraum).
 \item \label{Bez12}
       Sei 
...
wenn man beide Räume mit Normen ausstattet.
 \item \label{Bez14} Ist $(M,d)$ ein metrischer Raum, $x \in M$ und $r \in
  \R_{>0}$, so seien
  \[\koff M x r \coloneqq  \menge{y \in X}{d(x,y) < r}\text{ und }
  \kabg M x r \coloneqq  \menge{y \in X}{d(x,y) \le r}\text.\]
  Ist $A \subseteq M$, so sei $\partial A \coloneqq  \overline A \setminus \Inn(A)$.
 \item \label{Bez15}
