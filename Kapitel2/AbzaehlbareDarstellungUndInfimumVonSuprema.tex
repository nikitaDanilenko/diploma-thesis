...
$\mci \coloneqq  \Pot_{ne}(I)$ und $\mcj \coloneqq  \Pot_{ne}(J)$.\\
Es gelte:
\begin{enumerate}[(a)]
...
\end{satz}
\begin{proof}
Definiere $W_j \coloneqq  \Inf{i\in I} X(i, j)$ für alle $j \in J$.\\
Wegen (a) ist $\menge{W_j(\omega)}{j\in J}$ für alle $\omega \in \Omega$ nach
oben beschränkt.\\
Für jedes $k\in J$ ist $W_k$ und
\begin{align}
\sup_{j \in J}W_j = \sup_{j \in J}\inf_{i \in I}X(i, j) \overset{(b)}= 
\inf_{i \in I}\sup_{j \in J}X(i, j)\quad \text{ nach (c) integrierbar.}
\label{feeeieq:1}
\end{align}
Damit sind die Voraussetzungen von~\ref{EndlicheDarstellungDesSupremumsIntegrierbarerFunktionen} für
$\folge W j J$ erfüllt. Daher ist
...
\label{feeeieq:2}
\end{align}
Seien nun $Z_i \coloneqq  \Sup{j \in J}Y(i, j)$ für alle $i \in I$.\\
Wegen (a) ist $\menge{Z_i(\omega)}{i\in I}$ nach unten beschränkt für alle
$\omega \in \Omega$.\\
Für alle $l\in I$ ist $Z_l$ und
\[\inf_{i \in I}Z_i = \inf_{i \in I}\sup_{j \in J}Y(i, j) \quad
\text{ nach (c) integrierbar.} \]
Damit sind auch die Vorausssetzungen von~\ref{EndlicheDarstellungDesInfimumsIntegrierbarerFunktionen} erfüllt. Also ist
