...
\end{lemma}
\begin{proof}
Ist die Integrierbarkeit von $f$ in den beiden Fällen geklärt, so folgt die
jeweils zweite Aussage einfach über die Monotonie des Erwartungswerts.\\
(1) Seien $f, g \ge 0$, $f\le g$ und $g$ integrierbar.\\
Sei $\varphi \in \mct(\Omega, \mfa, \R)$ mit $0 \le \varphi \le f$.\\
Dann ist auch $0 \le \varphi \le f \le g$ und daher
\[\menge{\E(\psi)}{\psi \in \mct(\Omega,\mfa,\R), 0 \le \psi\le f}
\subseteq
 \menge{\E(\psi)}{\psi \in \mct(\Omega,\mfa,\R), 0 \le \psi\le g}\text.\]
Die größere Menge ist allerdings beschränkt, weil $g$ integrierbar ist.\\
Also ist $\menge{\E(\psi)}{\psi \in \mct_1(\Omega,\mfa,\R), 0 \le \psi\le f}$
nichtleer und nach oben beschränkt, besitzt also ein Supremum in $\R$.\\
Daher
\begin{align*}
