...
$N(0, 2\sigma_k^2)$--verteilt. Damit sind die Voraussetzungen von~\ref{SatzVonFernique} für $\gamma$ und $\sqrt 2 \delta$ erfüllt, sodass gilt:
\[\E\left(\max_{i \in \haken n}\gamma_i \right)
...
$N(0,1)$--verteilte Zufallsgrößen, sowie $X_1 \coloneqq Y_1$ und $X_2\coloneqq -Y_1$.\\
...
\label{mzngeq:3}\text.
\end{align}
Seien $\Phi$ die Verteilung und $\varphi$ die Dichte von $N(0,1)$, sowie
$f$ die Dichte von $\max\meng{Y_1, Y_2}$.\\
Nach~\refclaim{EigenschaftenVonOrdnungsstatistiken}
{EigenschaftenVonOrdnungsstatistiken4} gilt dann: $f = 2\Phi\varphi$,
sodass wir wegen $\forall\, t \in \R: \varphi'(t) = -t\varphi(t)$ erhalten:
\begin{align}
\E\left(\max_{i \in \haken 2}Y_i \right)&=
\int_{-\infty}^\infty t f(t)dt = 2\int_{-\infty}^\infty \Phi(t)t\varphi(t)dt
= - 2\int_{-\infty}^\infty \Phi(t)\varphi'(t)dt\notag\\
&= -2\left([\Phi(t)\varphi(t)]_{t=-\infty}^\infty -
\int_{-\infty}^\infty \varphi(t)^2dt \right)
...
\sqrt 2 \E\left(\max_{i \in \haken 2}Y_i \right)\text. \qedhere\]
\end{proof}