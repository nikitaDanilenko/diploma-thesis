...
Seien
\[M_{k, x}:=\menge{x_i}{i\in\haken n,\abs{\menge{j\in\haken n}{x_j>x_i}}<k}\]
und
\[M_{k+1,x}:=
...
"`$\Rightarrow$"': Es gelte $\mu_{k,n}(x) \ge a$.\\
...
"`$\Leftarrow$:"' Es gelte $\mu_{k,n}(x) < a$.\\
...
= \bigcup_{\substack{A \subseteq \haken n,\\ \abs A \ge k}}
\bigcap_{i \in \haken A} \pr_i\inv(]a, \infty[) =: G\text.
\end{align*}
...
= \bigcup_{\substack{B \subseteq \haken n,\\ \abs B > n - k}}
\bigcap_{i \in \haken B} \pr_i\inv(]-\infty, b[) =: K\text.
\end{align*}
Auch $K$ ist (mit dem gleichen Argument wie $G$) offen. Außerdem ist
