...
Dann existiert ein $\varphi \in \kran{E'}01$
mit $\norm x_E = \varphi(x)$.\\
Insbesondere ist damit $\opnorm {ev(x, \cdot)} {E'} \K  = \norm x_E$.\\
Für alle $K \in \meng{\kabg{E'}01, \kran{E'}01}$
gilt daher $\norm x _ E = \sup\menge{\abs{\varphi(x)}}{\varphi \in K}$.\\
Für $\K = \R$ gilt außerdem $\norm x _ E = \sup\menge{\varphi(x)}{\varphi \in K}$.
\end{lemma}
\begin{proof}
...
$l \in Y'$.\\
Dann existiert ein $L \in X'$ mit $\restrict{L}{Y} = l$ und $\opnorm L X \K = \opnorm l Y \K$.
\end{quote}
...
$\opnorm l F \K = 1$.\\
Also existiert nach obigem Satz ein $L \in E'$ mit $\restrict{L}{F} = l$ und
$\opnorm L E \K = 1$.\\
Wegen $x \in F$ gilt weiter $L(x) = L(\norm x_E \cdot x') = \norm x_E$, womit
der erste Teil der Behauptung gezeigt ist.\\
Seien $K \in \meng{\kabg{E'}01, \kran{E'}01}$ und $\varphi \in K$.\\
Dann gilt $\abs{\varphi(x)} \le \opnorm \varphi E \K \cdot \norm x_E \le \norm x_E$.\\
