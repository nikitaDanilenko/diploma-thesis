\begin{lemma}{Abschätzung der unvollständigen Beta--Funktion}
...
\begin{enumerate}[(1)]
	...
	\int_{y}^\infty tu(t)^{n-l}(1-u(t))^l dt \le
	\int_0^{u(y)}t^{n-l-1}(1-t)^l dt$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
Somit ist $\alpha'(s)>0$ für alle $s \in\!\  ]0,x[$, also
...
\begin{align*}
&\int_y^\infty t u(t)^{n-l}(1-u(t))^l dt
...
\int_{]0,u(y)[}t^{n-l-1}(1-t)^l dt = \int_0^{u(y)} t^{n-l-1}(1-t)^l dt
\end{align*}
\end{proof}